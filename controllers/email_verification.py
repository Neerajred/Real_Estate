...
class AgentController(http.Controller):
    ...
    def submit_registration(self, **post):
        ...
        return request.render('Real_Estate.registration_successful', {'email': email})

    @http.route('/agent/registration', type='http', auth='public', website=True)
    def display_agent_registration_form(self, **kwargs):
        return request.render("Real_Estate.agent_registration_form_template")
    ...
    def verify_email(self, email, **kwargs):
        ...
        if not agents:
            return request.render('Real_Estate.email_verification_failed')

        for agent in agents:
            if agent.email_verified:
                return request.render('Real_Estate.email_already_verified')
            agent.verify_email()

        return request.render('Real_Estate.email_verification_success')
